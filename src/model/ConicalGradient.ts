...
export class ConicalGradient {
  ...
  fillCircle(ctx: CanvasRenderingContext2D, centerX: number, centerY: number, innerX: number, outerX: number) {
        ...
        if (distance <= radius && distance >= innerX) {
          ...
          for (let i = 0; i < this.#limit; i++) {
            if (angle >= this.#fractions[i] && angle < this.#fractions[i + 1]) {
              pixColor = getRawColorFromFraction(
                this.#colors[i],
                this.#colors[i + 1],
                this.#fractions[i + 1] - this.#fractions[i],
                angle - this.#fractions[i]
              );
            }
          }
    ...
    const buffer = drawToCanvas(diameter, diameter, (_ctx) => {
  ...
  fillRect(
    ...
    const buffer = drawToCanvas(width, height, (_ctx) => {
      _ctx.putImageData(pixels, 0, 0);
    });
