...
export function createMeasuredValueImage(size: number, indicatorColor: string, radial: boolean, vertical: boolean) {
  console.warn(
    "[DEPRECATED] OLD__createMeasuredValueImage() is deprecated! use createMeasuredValuePointerImage(size, indicatorColor, linear?, vertical?) instead!"
  );
...
export function createThresholdImage(width: number, height: number, radial: boolean, vertical: boolean) {
  console.warn(
    "[DEPRECATE] OLD__createThresholdImage() is deprecated! Use createThresholdPointerImage(width, height, linear?, vertical?"
  );
...
export function drawLinearIndicator(
  ...
  console.warn(
    "[DEPRECATED] OLD__drawLinearIndicator() is deprecated! Use drawLinearIndicator(ctx, indicator, options) instead!"
  );
  NEW__drawLinearIndicator(ctx, indicator, { value, minValue, maxValue, gaugeType, vertical, altPos });
}

export function drawPointerImage(
  ctx: CanvasCtx,
  size: number,
  ptrType: PointerTypeDef,
  ptrColor: ColorDef,
  lblColor: RgbaColor
) {
