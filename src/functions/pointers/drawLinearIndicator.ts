...
export function drawLinearIndicator(ctx: CanvasCtx, indicator: HTMLCanvasElement, options: Options) {
  ...
  if (options.vertical) {
    yOffset = options.gaugeType.type === "type2" ? 0.7475 : 0.856796;
    yRange = yOffset - 0.12864;
    valuePos =
      height * yOffset - (height * yRange * (options.value - options.minValue)) / (options.maxValue - options.minValue);
    posX = options.altPos ? width * 0.34 - indicator.width : width * 0.365;
    posY = valuePos - indicator.height / 2;
  } else {
    const factor = options.gaugeType.type === "type2" ? 0.19857 : 0.142857;
    yOffset = options.gaugeType.type === "type2" ? 0.82 : 0.871012;
