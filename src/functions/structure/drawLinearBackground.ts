...
export function drawLinearBackground(ctx: CanvasCtx, options: Options) {
  ...
  if (!(CACHE_KEY in cache)) {
    ...
    const frameWidth =
      Math.ceil(Math.min(Math.sqrt(width * width + height * height) * 0.04, (vertical ? width : height) * 0.1)) - 1;
    ...
    switch (options.color.name) {
      ...
      case "STAINLESS":
        ...
        bufferCtx.fillStyle = createLinearGradient(
          bufferCtx,
          frameWidth,
          frameWidth,
          width - frameWidth * 2,
...
function drawStainlessBackground(ctx: CanvasCtx, width: number, height: number, offset: number) {
  ...
  grad.fillRect(ctx, width / 2, height / 2, width - offset * 2, height - offset * 2, width / 2, height / 2);
...
function drawTurnedEffect(ctx: CanvasCtx, width: number, height: number, offset: number, cornerRadius: number) {
  ...
  const radius =
    Math.sqrt((width - offset * 2) * (width - offset * 2) + (height - offset * 2) * (height - offset * 2)) / 2;
