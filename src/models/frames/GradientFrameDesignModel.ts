import FrameDesignModel, { DrawOptions } from "./FrameDesignModel";
import { TWO_PI } from "../../shared";

export default abstract class GradientFrameDesignModel extends FrameDesignModel {
  draw(ctx: CanvasRenderingContext2D, {
    ...
    // main gradient frame
    this.drawGradient(ctx, { width, height, centerX, centerY });
    ...
    //   case "tiltedGray":
    //     radFCtx.fillStyle = createLinearGradient(
    //       radFCtx,
    //       0.233644 * width,
    //       0.084112 * height,
    //       0.81258 * width,
    ...
    ctx.beginPath();
    ctx.arc(centerX, centerY, (width * 0.841121) / 2, 0, TWO_PI, true);
    ctx.closePath();
    ctx.fill();
    ...
    // Background ellipse
    ctx.beginPath();
    ctx.arc(centerX, centerY, (width * 0.83) / 2, 0, TWO_PI, true);
    ctx.closePath();
    ctx.fill();
  }

  protected abstract drawGradient(ctx: CanvasRenderingContext2D, options: Required<DrawOptions>): void;
}